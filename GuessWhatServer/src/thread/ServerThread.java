...
public class ServerThread extends Thread {
	...
	public void run() {
		try {
				...
				try {
					...
					if (message != null) {
						...
						if (requestTokens[0].equals(Request.LOGIN.getRequest())) {// Login:Id
							...
							this.addBanManager(requestTokens[1], requestTokens[2], requestTokens[3], requestTokens[4],
									requestTokens[5]);
							...
							this.addStudent(requestTokens[1], requestTokens[2], requestTokens[3], requestTokens[4],
									requestTokens[5]);
			...
			pw.println(clientRequest + ":" + s.getMessage());
	...
	private void addProblem(String problems) {
		...
		for (int i = 0; i < problemList.length; i++) {
			...
			if (!DB_Problem.insertProblem(problemInfo[0], problemInfo[1], problemInfo[2], problemInfo[3],
					problemInfo[4])) {
	...
	private void deleteBanManager(String PNum, String BNum, String BMNum) {
		...
		if (DB_BanManager.deleteBanManager(pNum, bNum, bmNum))
			...
			pw.println("DeleteBanManager:Fail");
	...
	private void deleteWorkbook(String WNum) {
		if (DB_Workbook.deleteWorkbook(WNum))
			...
			pw.println("DeleteWorkbook:Fail");
	...
	private void modifyWorkbook(String WNum, String newName) {
		if (DB_Workbook.modifyWorkbookName(WNum, newName))
			...
			pw.println("ModifyWorkbook:Fail");
	...
	private void getAllBanManager(String PNum, String BNum) { // GetAllBanManager:Success:Name, State, Code, WorkBook
		...
		List<BanManager> listBanManager = DB_BanManager.getAllBanManager(Integer.parseInt(PNum),
				Integer.parseInt(BNum));
		...
		if (listBanManager == null) {
			...
			while (iterator.hasNext()) {
				...
				result = result + ":" + banManager.BM_num() + ":" + banManager.BM_name() + ":"
						+ banManager.stateToString(banManager.BM_sate()) + ":" + banManager.BM_roomcode() + ":"
						+ banManager.workbook() + ":" + banManager.student_size();
			}
			pw.println(result);
			pw.flush();
		}
	...
	private void getCurrentBanManager(String PNum, String BMNum) { // GetCurrentBanManager:Success:Name, State, Code,
																	// WorkBook
	...
	private void getProblem(String WNum, String index) {
		...
		if (problem == null)
			...
			pw.println("GetProblem:Success:" + problem.tokenString());
		pw.flush();
	}
